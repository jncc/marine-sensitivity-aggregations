...
# Aggregate MarESA sensitivity assessments for UK offshore deep sea broadscale habitats.
...
def main(marESA_file, EnglishOffshore):
    ...
    columns_of_interest = ['BSH', 'JNCC code', 'JNCC name', 'Pressure',
    ...
    bsh_agg = bsh_maresa_unknowns.groupby(['Pressure', 'BSH'])['Sensitivity'].apply(lambda x: ', '.join(x)).reset_index(inplace=False)
