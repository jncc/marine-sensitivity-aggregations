...
def main(marESA_file, EngWel_Annex1):
    ...
    # Annex 1 sub-type data - English_Welsh_Offshore_AnnexI
    ...
    #############################################################
    # Merge MarESA sensitivity assessments with all Habitats Directive
    # listed Annex 1 habitats and sub-types of relevance
    ...
    #############################################################
    # Step 1: Assign a full set of pressures with the value 'Unknown'
    # to all annex data which do not have MarESA Assessments
    ...
    maresa_annex_agg = annex_maresa_unknowns.groupby(
        ['Pressure', 'unique_id'])['Sensitivity'].apply(
            lambda x: ', '.join(x)).reset_index(inplace=False)
    ...
    maresa_annex_agg['AggregationConfidenceScore'] = maresa_annex_agg.apply(lambda df: ml.categorise_confidence(df, 'AggregationConfidenceValue'), axis=1)
    ...
    # maresa_annex_agg = maresa_annex_agg[[
    #     'Pressure', 'SubregionName', 'Annex I habitat', 'Annex I sub-feature',
    #     'AggregatedSensitivity', 'AssessedCount', 'UnassessedCount',
    #     'AggregationConfidenceValue', 'AggregationConfidenceScore'
    # ]]
