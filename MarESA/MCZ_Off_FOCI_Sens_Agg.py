# Title: MCZ Offshore Feature of Conservation Importance (FOCI) Sensitivity Aggregation

# Aggregate MarESA sensitivity assessments for offshore MCZ Features of Conservation Importance.
...
def main(marESA_file, EnglishOffshore):
    ...
    columns_of_interest = ['FOCI', 'SubregionName', 'EUNIS name', 'JNCC code',
    ...
    foci_maresa_unknowns.replace(reformat_ev, inplace=True)

    # Fill NaN values with empty string values to allow for string formatting with unwanted_char() function
    foci_maresa_unknowns['Sensitivity'].fillna(value='', inplace=True)
    ...
    foci_maresa_unknowns['unique_id'] = foci_maresa_unknowns['FOCI'].astype(str) + ':'\
                                         + foci_maresa_unknowns['SubregionName'].astype(str)

    # Aggregate data together by unique instances of FOCI and Bioregion
    foci_agg = foci_maresa_unknowns.groupby(['Pressure', 'unique_id'])['Sensitivity'].apply(lambda x: ', '.join(x)).reset_index(inplace=False)
    ...
    foci_agg[[
        ...
        'Unknown'
    ]] = foci_agg.apply(lambda df: pd.Series(ml.counter(df['Sensitivity'])), axis=1)
    ...
    foci_agg[['Feature of Conservation Importance (FOCI)', 'Bioregion']] = foci_agg.apply(lambda row: ml.split_id2(row), axis='columns', result_type='expand')

    # Reorder columns within DF
    foci_agg = foci_agg[
        ['Pressure', 'Feature of Conservation Importance (FOCI)', 'Bioregion', 'AggregatedSensitivity', 'AssessedCount',
