...
# Script description:    Aggregate MarESA resilience assessments for UK offshore deep sea broadscale habitats.
...
def main(marESA_file, EnglishOffshore,output_file):
    ...
    bsh = bsh[bsh["BSH"] == "Deep-sea bed"]
    bsh.drop('FOCI', axis=1, inplace=True)
    bsh.drop_duplicates(inplace=True)
    ...
    bsh_maresa_unknowns['Resilience'].fillna(value='', inplace=True)
    ...
    bsh_maresa_unknowns.loc[bsh_maresa_unknowns['Resilience'] == 'Very Low', 'Resilience'] = 'Very low'
    # Test this has worked - check values using .unique() on column of interest
    bsh_maresa_unknowns.Resilience.unique()
    ...
    bsh_agg = bsh_maresa_unknowns.groupby(['Pressure', 'BSH'])['Resilience'].apply(lambda x: ', '.join(x))
    ...
    bsh_agg[[
        ...
        'Not assessed', 'Unknown']] = bsh_agg.apply(lambda df: pd.Series(counter(df['Resilience'])), axis=1)
    ...
    bsh_agg['Count_vLow'] = bsh_agg['Very low']
    ...
    bsh_agg['AggregatedResilience'] = bsh_agg.apply(lambda df: final_resilience(df), axis=1)
    ...
    bsh_agg = bsh_agg[
        ['Pressure', 'BSH', 'AggregatedResilience', 'AssessedCount', 'UnassessedCount', 'AggregationConfidenceValue',
